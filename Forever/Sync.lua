-- SyncEvents(channel, target, event, ...) -> ForeverAuras.ScanEvents(event, ...)
-- Receive channels: PARTY, RAID, INSTANCE_CHAT, GUILD, OFFICER
-- Prefix: WA_SYNC_EVENTS (kept for existing packs)
...
local function receiveSyncEvent(prefix, encoded, channel, sender)
    ...
    ForeverAuras.ScanEvents(data["event"], unpack(data["payload"]))
end
